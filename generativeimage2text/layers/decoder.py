...
class CaptioningModel(nn.Module):
    def __init__(
        ...
        if num_image_with_embedding:
            logging.info(f"creating temperal embedding {num_image_with_embedding}")
    ...
    def forward(self, batch):
        # do split images here
        if batch['image'].dim() == 5:
            im = batch['image']
            six = []
            for i in range(im.size(1)):# get num_images
                six.append(im[:,i,:,:,:])
            batch['image'] = six
    ...
    def forward_one(self, batch, return_info=False):
        ...
        if 'image' in batch:
            # b = batch['image']
            # print('in forward here\n')
            # batch['image'] = [b.clone()]*6
            # print(batch['image'][0].shape) torch.Size([1, 3, 160, 160])
